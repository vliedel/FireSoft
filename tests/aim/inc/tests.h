...
private:
  ...
  // private storage for portStatusValue
  int portStatusValue;
...
public:
  ...
  void Init(std::string module_id) {
    ...
    
    {
      ...
      portName << "/tests" << module_id << "/command";
      portCommand->open(portName.str().c_str());
    }
    {
      std::stringstream portName; portName.str(); portName.clear();
      portName << "/tests" << module_id << "/status";
...
protected:
  ...
  inline int *readStatus(bool blocking=true) {
    Bottle *b = portStatus->read(blocking);
    if (b != NULL) { 
      portStatusValue = b->get(0).asInt();
